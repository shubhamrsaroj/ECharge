<template>
  <div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
      ...
      <h1 class="text-3xl font-bold text-white"><span class="text-primary-400 text-shadow-neon">Edit</span> Charging Station</h1>
    </div>

    <div class="bg-dark-300 rounded-lg shadow-md overflow-hidden border border-dark-200">
      <div class="p-6">
        <!-- Loading State -->
        <div v-if="loading && !station" class="flex justify-center py-12">
        ...
        <!-- Edit Form -->
        <form v-else @submit.prevent="submitForm" class="text-gray-300">
          <!-- Error Alert -->
          <div v-if="formError" class="bg-red-900/30 border border-red-800 text-red-300 px-4 py-3 rounded mb-6" role="alert">
          ...
          <div class="flex justify-end space-x-4">
            <button type="button" @click="$router.push(`/stations/${stationId}`)" class="btn btn-secondary">
              Cancel
            </button>
            <button type="submit" class="btn btn-primary" :disabled="formLoading">
              <svg v-if="formLoading" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
...
const fetchStation = async () => {
  try {
    ...
    form.status = station.value.status;
    form.location.address = station.value.location.address;
    form.location.city = station.value.location.city;
    form.location.state = station.value.location.state;
    form.location.zipCode = station.value.location.zipCode;
    form.location.country = station.value.location.country;
    form.location.coordinates = [...station.value.location.coordinates];
    form.powerOutput = station.value.powerOutput;
    form.connectorTypes = [...station.value.connectorTypes];
    form.openingHours = station.value.openingHours;
    form.pricing.perKwh = station.value.pricing.perKwh;
    form.pricing.perMinute = station.value.pricing.perMinute;
    form.pricing.currency = station.value.pricing.currency;
    form.amenities = station.value.amenities ? [...station.value.amenities] : [];
    form.photos = station.value.photos ? [...station.value.photos] : [];
    
    // Set coordinates
    longitude.value = station.value.location.coordinates[0];
    latitude.value = station.value.location.coordinates[1];
    
    // Set photos text
    photosText.value = station.value.photos ? station.value.photos.join('\n') : '';
    
    // Check if user is authorized to edit
    if (!authStore.isAdmin && (!station.value.owner || station.value.owner._id !== authStore.user?.id)) {
      router.push('/stations');
    }
  } catch (err) {
    error.value = 'Failed to load station details. Please try again.';
...
const submitForm = async () => {
  try {
    ...
    // Submit to API
    const result = await stationStore.updateStation(stationId, form);
    
    // Redirect to the station's details page
    router.push(`/stations/${stationId}`);
  } catch (err) {
    formError.value = err.response?.data?.message || 'Failed to update charging station. Please try again.';
